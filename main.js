...
function buildReservationEmailParams(baseReservation, extras = {}) {
  ...
  return {
    ...
    review_text: 'Après votre course, laissez-nous un avis sur Google en recherchant Taxi Etoile. Merci pour votre confiance.',
...
function reservationWhatsAppUrl(item) {
  ...
  if (!phone) return '';
  const text = `Bonjour ${reservationName(item) || ''}, Taxi Etoile pour votre course du ${formatDateTime(reservationDateTime(item))}. Départ: ${reservationPickup(item) || '—'} | Arrivée: ${reservationDropoff(item) || '—'}.`;
...
function exportReservationsCsv() {
  ...
  a.href = url;
  a.download = 'dispatch-taxi-etoile.csv';
  a.click();
